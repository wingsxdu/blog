...
虽然 unix 及其变种操作系统之间的实现方式有些许不同，但是`ISO、IEEE`等组织对其进行了一系列标准化规范。文中将以 Linux 的实现方式为主要介绍目标。在此之前，需要介绍几个概念。
...
其中内核空间由内核专用，而用户进程的虚拟内存会被分配到用户空间中，通常情况下这些用户进程的虚拟内存之间相互独立，彼此不可见。进程的虚拟内存又会被内核划分为若干**页**（page），而物理内存的划分由 CPU 负责，一个物理内存单元被称为**页框**（page frame）。进程的不同页会被映射到对应的物理内存之上。如下图所示：
...
内核线程的管理成本要比用户线程高很多，线程的创建、切换、同步等过程也会消耗更多的资源。如果一个进程包含了大量的线程，也会给内核的调度器带来很大的负担。一般情况下，操作系统会对一个进程中可以创建的线程数量有直接或间接的限制。
...
这里提一个题外话，关于 CPU 的超线程技术。对于处理器核心来说，虽然可以每秒钟处理成千上万条指令，但是在某一时刻，只能够对一条指令进行处理，但一般情况下单条指令不会利用 CPU 内部的所有资源。超线程是一种可以将 CPU 内部暂时闲置的处理资源充分“调动”起来的技术，超能够把一个物理处理器在软件层变成两个逻辑处理器，可以使处理器在某一时刻，同步并行处理更多指令和数据，当然实际效能不是实现双倍提升。从时间角度看，一个拥有两个逻辑处理器的物理核心同时执行的任务数量也是两个。
