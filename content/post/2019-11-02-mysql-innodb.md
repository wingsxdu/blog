...
![mysql](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\mysql.png)
...
![Tablespace-segment-extent-page-row](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Tablespace-segment-extent-page-row.jpg)
...
![frm-and-ibd-file](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\frm-and-ibd-file.jpg)
...
![frm-file-hex](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\frm-file-hex.png)
...
![Antelope-Barracuda-Row-Format](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Antelope-Barracuda-Row-Format.jpg)

> Antelope 是 InnoDB 最开始支持的文件格式，它包含两种行格式 Compact 和 Redundant，它最开始并没有名字；Antelope 的名字是在新的文件格式 Barracuda 出现后才起的，Barracuda 的出现引入了两种新的行格式 Compressed 和 Dynamic；InnoDB 对于文件格式都会向前兼容，而官方文档中也对之后会出现的新文件格式预先定义好了名字：Cheetah、Dragon、Elk 等等。

两种行记录格式 Compact 和 Redundant 在磁盘上按照以下方式存储：

![COMPACT-And-REDUNDANT-Row-Format](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\COMPACT-And-REDUNDANT-Row-Format.jpg)
...
![Row-Overflo](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Row-Overflow.jpg)

但是当我们使用新的行记录格式 Compressed 或者 Dynamic 时都只会在行记录中保存 20 个字节的指针，实际的数据都会存放在溢出页面中。

![Row-Overflow-in-Barracuda](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Row-Overflow-in-Barracuda.jpg)
...
![InnoDB-B-Tree-Node](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\InnoDB-B-Tree-Node.jpg)
...
![Infimum-Rows-Supremum](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Infimum-Rows-Supremum.jpg)
...
![B+Tree](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\B+Tree.jpg)
...
![Clustered-Index](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Clustered-Index.jpg)
...
![Secondary-Index](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Secondary-Index.jpg)

如果在表 `users` 中存在一个辅助索引 `(first_name, age)`，那么它构成的 B+ 树大致就是上图这样，按照 `(first_name, age)` 的字母顺序对表中的数据进行排序，当查找到主键时，再通过聚集索引获取到整条行记录。

![Clustered-Secondary-Index](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Clustered-Secondary-Index.jpg)

上图展示了一个使用辅助索引查找一条表记录的过程：通过辅助索引查找到对应的主键，最后在聚集索引中使用主键获取对应的行记录，这也是通常情况下行记录的查找方式。

### 索引的设计

索引的设计其实是一个非常重要的内容，同时也是一个非常复杂的内容；索引的设计与创建对于提升数据库的查询性能至关重要，不过这不是本文想要介绍的内容，有关索引的设计与优化可以阅读 [数据库索引设计与优化](https://draveness.me/数据库索引设计与优化) 一书，书中提供了一种非常科学合理的方法能够帮助我们在数据库中建立最适合的索引，当然作者也可能会在之后的文章中对索引的设计进行简单的介绍和分析。
...
![Optimistic-Pessimistic-Locks](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Optimistic-Pessimistic-Locks.jpg)
...
![Shared-Exclusive-Lock](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Shared-Exclusive-Lock.jpg)
...
![Lock-Type-Compatibility-Matrix](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Lock-Type-Compatibility-Matrix.jpg)
...
![Deadlocks](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Deadlocks.jpg)
...
![Transaction-Isolation-Matrix](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Transaction-Isolation-Matrix.jpg)
...
![Read-Uncommited-Dirty-Read](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Read-Uncommited-Dirty-Read.jpg)
...
![Read-Commited-Non-Repeatable-Read](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\2019-07-16-Read-Commited-Non-Repeatable-Read.jpg)
...
![Repeatable-Read-Phantom-Read](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Repeatable-Read-Phantom-Read.jpg)

这种现象在数据库中就被称作幻读，虽然我们使用查询语句得到了一个空的集合，但是插入数据时却得到了错误，好像之前的查询是幻觉一样。

在标准的事务隔离级别中，幻读是由更高的隔离级别 `SERIALIZABLE` 解决的，但是它也可以通过 MySQL 提供的 Next-Key 锁解决：

![Repeatable-with-Next-Key-Lock](C:\Users\beihai\Desktop\WorkSpace\blog\static\image\Repeatable-with-Next-Key-Lock.jpg)
