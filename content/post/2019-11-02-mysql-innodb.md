...
<div align="center">{{< figure src="/image/Database-Instance.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Tablespace-segment-extent-page-row.jpg">}}</div>
同一个数据库实例的所有表空间都有相同的页大小；默认情况下，表空间中的页大小都为 16KB，当然也可以通过改变 `innodb_page_size` 选项对默认大小进行修改，需要注意的是不同的页大小最终也会导致区大小的不同：

<div align="center">{{< figure src="/image/Relation%20Between%20Page%20Size%20-%20Extent%20Size.png">}}</div>
...
<div align="center">{{< figure src="/image/frm-and-ibd-file.jpg">}}</div>
#### .frm 文件{###.frm 文件}
...
<div align="center">{{< figure src="/image/frm-file-hex.png">}}</div>
...
<div align="center">{{< figure src="/image/Antelope-Barracuda-Row-Format.jpg">}}</div>
> Antelope 是 InnoDB 最开始支持的文件格式，它包含两种行格式 Compact 和 Redundant，它最开始并没有名字；Antelope 的名字是在新的文件格式 Barracuda 出现后才起的，Barracuda 的出现引入了两种新的行格式 Compressed 和 Dynamic；InnoDB 对于文件格式都会向前兼容，而官方文档中也对之后会出现的新文件格式预先定义好了名字：Cheetah、Dragon、Elk 等等。

两种行记录格式 Compact 和 Redundant 在磁盘上按照以下方式存储：

<div align="center">{{< figure src="/image/COMPACT-And-REDUNDANT-Row-Format.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Row-Overflow.jpg">}}</div>
但是当我们使用新的行记录格式 Compressed 或者 Dynamic 时都只会在行记录中保存 20 个字节的指针，实际的数据都会存放在溢出页面中。

<div align="center">{{< figure src="/image/Row-Overflow-in-Barracuda.jpg">}}</div>
...
<div align="center">{{< figure src="/image/InnoDB-B-Tree-Node.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Infimum-Rows-Supremum.jpg">}}</div>
...
<div align="center">{{< figure src="/image/B+Tree.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Clustered-Index.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Secondary-Index.jpg">}}</div>
如果在表 `users` 中存在一个辅助索引 `(first_name, age)`，那么它构成的 B+ 树大致就是上图这样，按照 `(first_name, age)` 的字母顺序对表中的数据进行排序，当查找到主键时，再通过聚集索引获取到整条行记录。

<div align="center">{{< figure src="/image/Clustered-Secondary-Index.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Optimistic-Pessimistic-Locks.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Shared-Exclusive-Lock.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Lock-Type-Compatibility-Matrix.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Deadlocks.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Transaction-Isolation-Matrix.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Read-Uncommited-Dirty-Read.jpg">}}</div>
### 不可重复读{##不可重复读}
...
<div align="center">{{< figure src="/image/2019-07-16-Read-Commited-Non-Repeatable-Read.jpg">}}</div>
...
<div align="center">{{< figure src="/image/Repeatable-Read-Phantom-Read.jpg">}}</div>
这种现象在数据库中就被称作幻读，虽然我们使用查询语句得到了一个空的集合，但是插入数据时却得到了错误，好像之前的查询是幻觉一样。

在标准的事务隔离级别中，幻读是由更高的隔离级别 `SERIALIZABLE` 解决的，但是它也可以通过 MySQL 提供的 Next-Key 锁解决：

<div align="center">{{< figure src="/image/Repeatable-with-Next-Key-Lock.jpg">}}</div>
