...
![Basic-Paxos-Consistency@2x](Basic-Paxos-Consistency@2x.png)
...
本篇文章先介绍了一些重要的理论，其中 FLP 不可能定理指出不要为分布式系统设计出一个通用的共识算法，『银弹』是不存在的，CAP 和 BASE 则针对一些特定的应用范围进行了讨论。文中主要总结了三类共识算法：实现了原子提交的 2PC 及其变种 3PC、支持容错的类 Paxos 和解决拜占庭将军问题的工作量证明。从这里不难看出，可扩展性并不是使用分布式系统的唯一原因，支持容错与低延迟也是同样重要的目标。并且它们都有一个共同的特点：使用主从复制来提高系统的扩展性，在工程中将目光集中到维护『主从复制』这个点上或许会有所收获 。
